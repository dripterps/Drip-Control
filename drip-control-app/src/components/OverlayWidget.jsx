...
// exactly what ships.
export default function OverlayWidget({ bagsLeft, totalBags, variant = "full", sizeTier = "lg", opacity = 0.55 }) {
  ...
  const hasGlass = clampedOpacity > 0;
  ...
  return (
    <div
      ...
      style={{
        ...
        background: hasGlass ? `rgba(20, 19, 26, ${clampedOpacity})` : "transparent",
        border: hasGlass ? "1px solid rgba(255,255,255,0.10)" : "none",
        backdropFilter: hasGlass ? "blur(18px)" : "none",
        WebkitBackdropFilter: hasGlass ? "blur(18px)" : "none",
        boxShadow: hasGlass ? `0 0 0 1px rgba(255,255,255,0.03), 0 12px 40px rgba(0,0,0,0.45), 0 0 40px ${status.dot}22` : "none",
        minWidth: s.minW,
      }}
    >
      <div className="mb-2">
        <span className="dc-font-mono" style={{ fontSize: s.label, letterSpacing: "0.16em", color: C.textPrimary, fontWeight: 600 }}>
          BAGS LEFT
        </span>
      </div>
      <div className="flex items-baseline gap-1.5 mb-2.5">
        ...
        />
        <span className="dc-font-display" style={{ fontSize: s.denom, color: C.textPrimary, fontWeight: 600 }}>/ {totalBags}</span>
      </div>
